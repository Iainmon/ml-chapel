...
mkdir -p speed_tests3

echo "data_size,num_seconds" > speed_tests3/mnist_trainer.py.csv
echo "data_size,num_seconds" > speed_tests3/mnist_trainer.chpl.csv

# Run the python tests
for data_size in 20 40 60 80 100 120 140 160 180 200; do
    # Print indication of progress
    echo "Running data_size=$data_size"

    # Run the command and capture its time
    time_output=$(python3.6 mnist_trainer.py $data_size | tail -n 1)
...
for data_size in 20 40 60 80 100 120 140 160 180 200; do
    ...
    # Run the command and capture its time
    time_output=$(./mnist_trainer --dataSize=$data_size | tail -n 1)
